...
export class TipoProductoListComponent implements AfterViewInit {
  ...
  readonly dataSource = new MatTableDataSource<TipoProductoRead>([]);
  loading = true;
  ...
  editar(row: TipoProductoRead): void {
    this.open({ mode: 'edit', row });
  }

  eliminar(row: TipoProductoRead): void {
  ...
  private open(data: TipoProductoDialogData): void {
    this.dialog
      .open(TipoProductoDialogComponent, {
        width: '720px',
        maxWidth: '96vw',
